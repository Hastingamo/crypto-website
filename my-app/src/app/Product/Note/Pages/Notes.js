...
function Notes() {
  ...
  useEffect(() => {
    const stored = localStorage.getItem("selectedTemplateFront");
    if (stored) {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("noteElementsFront");
    if (saved) setElements(JSON.parse(saved));
  }, []);

  useEffect(() => {
    localStorage.setItem("noteElementsFront", JSON.stringify(elements));
  }, [elements]);
  ...
  const handleBackClick = () => {
    localStorage.removeItem("selectedTemplateFront");
    localStorage.removeItem("noteElementsFront");
          ...
          >
            {el.type === "text" ? (
             ...
             text-black dark:text-white border-none outline-none resize-none"
                ...
                <span
                  className="text-black dark:text-white w-[90%] break-words cursor-text"
                  onClick={(e) => {
                    e.stopPropagation();
                    setCurrent(el.id);
                  }}
