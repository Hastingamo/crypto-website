...
//   const selectedPair = data.find(
//     (p) => p.symbol?.replace(/_/g, "").toUpperCase()
//   );
...
//   router.push(`/Product/Forex/${selectedPair.symbol}`);
// };
...
function Page() {
  // const apikeys = "MYBRGA85QP1HBBAL";
  const apikey = process.env.NEXT_PUBLIC_PUBLIC_FINNHUB_API_KEY;
  const [data, setData] = useState([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState("");
  const [filtered, setFiltered] = useState([]);
